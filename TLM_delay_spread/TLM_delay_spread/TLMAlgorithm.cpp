...
void SingleIteration(ActiveNode **pActiveSet) 
{
	...
	//	if (SQUARE(Value) > 0.01*Grid[x][y][z].Energy) {
			Grid[x][y][z].Energy += SQUARE(Value);
			Grid[x][y][z].Em += nIterations * SQUARE(Value);
			Grid[x][y][z].Emm += SQUARE(nIterations) * SQUARE(Value);
	//	}
		
		// Assign to the node
		Grid[x][y][z].V = Value;

		if (AvgEnergy < AbsoluteThreshold || AvgEnergy < NodeReference->Energy*RelativeThreshold) {
...
void EvaluateSource(int Iteration)
{
	...
	AvgEnergy = SQUARE(V) + SQUARE(Grid[ImpulseSource.X][ImpulseSource.Y][ImpulseSource.Z].V);

	Grid[ImpulseSource.X][ImpulseSource.Y][ImpulseSource.Z].V = V;

	Grid[ImpulseSource.X][ImpulseSource.Y][ImpulseSource.Z].Energy += SQUARE(V);
}
