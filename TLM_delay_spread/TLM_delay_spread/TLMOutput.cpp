	...
	if (fopen_s(&PathLossFile, FilenameBuffer, "w") != 0) {
		...
		switch (PathLossParameters.Type) {
			...
			case POINT: {
				...
				AvgDelay = Grid[x][y][z].Em/Grid[x][y][z].Energy;
				DelaySpread = GridSpacing/SPEED_OF_LIGHT * sqrt(Grid[x][y][z].Emm/Grid[x][y][z].Energy - SQUARE(AvgDelay));
				AvgDelay *= GridSpacing/SPEED_OF_LIGHT;
				PathLoss = VoltageToDB(SPEED_OF_LIGHT/Frequency * sqrt(Grid[x][y][z].Energy) * KAPPA/4/M_PI/GridSpacing);
		...
		for (int xx = 0; xx < 10; xx++) { 
			for (int yy = 0; yy < 10; yy++) {
				for (int zz = 0; zz < 10; zz++) {
					...
					fprintf(OutputFile,"%f\t%f\t%f\t%f\n", Grid[x][y][z].Energy, d, asin((9.0*zz)/d), atan((double)(yy)/xx));
				}
			}
		}
