...
void Scatter(int ThreadNumber) 
{
	...
	ActiveNode *NewNode;
	int AdditionIndex = (ThreadNumber+Threads/2)%Threads;
	int x,y,z;
	...
	while (CurrentNode != NULL) {
		...
		if (x < (xSize-1)) {
			if (Grid[x+1][y][z].Active == false && Grid[x+1][y][z].PropagateFlag == true) {
				...
				NewNode->NextActiveNode = NodeAdditions[AdditionIndex][ThreadNumber];
				NodeAdditions[AdditionIndex][ThreadNumber] = NewNode;
				++AdditionIndex%=Threads;
			}
...
void CopyNodeAdditions(int ThreadNumber)
{
	...
	for (int i=0; i<Threads; i++) {
		...
		PreviousNode = NULL;
		CurrentNode = NodeAdditions[ThreadNumber][i];
		while (CurrentNode != NULL) {
			if (Grid[CurrentNode->X][CurrentNode->Y][CurrentNode->Z].Active == true) {
				...
				if (PreviousNode != NULL) {
					...
					NodeAdditions[ThreadNumber][i] = CurrentNode;
				}
			}
			else {
				ActiveJunctions[ThreadNumber]++;
		...
		if (NodeAdditions[ThreadNumber][i] != NULL) {
			if (PreviousNode != NULL) {
				PreviousNode->NextActiveNode = ActiveSet[ThreadNumber];
			}
			else {
				NodeAdditions[ThreadNumber][i]->NextActiveNode = ActiveSet[ThreadNumber];
			}
			ActiveSet[ThreadNumber] = NodeAdditions[ThreadNumber][i];
			NodeAdditions[ThreadNumber][i] = NULL;
		}
	}
}
...
void AllocateResources(void)
{
	...
	for (int i=0; i<Threads; i++) {
		...
		NodeAdditions[i] = (ActiveNode**)malloc(Threads*sizeof(ActiveNode*));
			
		for (int j=0; j<Threads; j++) {
			NodeAdditions[i][j] = NULL;
		}
