...
void MainLoop(void)
{
	...
	while (ActiveSet != NULL) {
		...
		if (nIterations == 100) {
			FILE *File;
			fopen_s(&File, "../OutputFiles/V100.txt","w");
			for (int x = 0; x< xSize; x++) {
				for (int y = 0; y< ySize; y++) {
					fprintf(File,"%f\t",Grid[x][y][zSize/2].V);
				}
				fprintf(File,"\n");
			}
			fclose(File);
...
{
	float Value;			// Temporary node value
...
void EvaluateSource(int Iteration)
{
	...
	switch (ImpulseSource.Type) {
		case IMPULSE:
			...
			break;
		case GAUSSIAN:
			V += exp(10*-SQUARE(((float)Iteration-ImpulseSource.Duration/2)/ImpulseSource.Duration));
			break;
		case RAISED_COSINE:
			V += 0.5*(1+cos(2*M_PI*((float)(Iteration+1)/(ImpulseSource.Duration)-0.5)));
			break;
	}
