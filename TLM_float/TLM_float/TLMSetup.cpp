...
extern float GridSpacing;
extern float AbsoluteThreshold;
extern float RelativeThreshold;
...
extern bool DefaultAbsoluteThreshold;
...
bool ReadFloat(char **Context, float *Float, bool *DefaultFlag);
			...
			if (fgets(Buffer, 200, InputFile) != NULL) {
				...
				if (ParameterName != NULL) {
					...
					// Read the grid spacing
						...
						if (ReadFloat(&Context, &GridSpacing, &DefaultGridSpacing) == false) {
							SuccessfulRead = false;
						}
					}
					...
					// Read the source position x
						...
						if (ReadFloat(&Context, &Position, &DefaultSourcePosition) == false) {
							SuccessfulRead = false;
						}
						else {
					...
					// Read the operating frequency
						...
						if (ReadFloat(&Context, &Frequency, &DefaultFrequency) == false) {
							SuccessfulRead = false;
						}
					}
					// Read the absolute voltage threshold
					else if (strcmp(ParameterName, "absolute_threshold") == 0) {
						if (ReadFloat(&Context, &AbsoluteThreshold, &DefaultAbsoluteThreshold) == false) {
							SuccessfulRead = false;
						}
					}
					// Read the relative voltage threshold
					else if (strcmp(ParameterName, "relative_threshold") == 0) {
						if (ReadFloat(&Context, &RelativeThreshold, &DefaultRelativeThreshold) == false) {
							SuccessfulRead = false;
						}
					}
					...
					// Read the path loss x1 coordinate
						...
						if (ReadFloat(&Context, &PathLossParameters.X1, &DefaultPLParams) == false) {
							SuccessfulRead = false;
						}
					}
					// Read the path loss y1 coordinate
					else if (strcmp(ParameterName, "pl_y1") == 0) {
						if (ReadFloat(&Context, &PathLossParameters.Y1, &DefaultPLParams) == false) {
							SuccessfulRead = false;
						}
					}
					// Read the path loss x2 coordinate
					else if (strcmp(ParameterName, "pl_x2") == 0) {
						if (ReadFloat(&Context, &PathLossParameters.X2, &DefaultPLParams) == false) {
							SuccessfulRead = false;
						}
					}
					// Read the path loss y2 coordinate
					else if (strcmp(ParameterName, "pl_y2") == 0) {
						if (ReadFloat(&Context, &PathLossParameters.Y2, &DefaultPLParams) == false) {
							SuccessfulRead = false;
						}
					}
					// Read the path loss height coordinate
					else if (strcmp(ParameterName, "pl_z") == 0) {
						if (ReadFloat(&Context, &PathLossParameters.Z1, &DefaultPLParams) == false) {
							SuccessfulRead = false;
						}
					}
					...
					// Read the path loss height coordinate 2 (cube)
						...
						if (ReadFloat(&Context, &PathLossParameters.Z2, &DefaultPLParams) == false) {
							SuccessfulRead = false;
						}
					}
					// Read the path loss spacing
					else if (strcmp(ParameterName, "pl_spacing") == 0) {
						if (ReadFloat(&Context, &PathLossParameters.Spacing, &DefaultPLParams) == false) {
							SuccessfulRead = false;
						}
					}
					...
					// Read the path loss y spacing
						...
						if (ReadFloat(&Context, &PathLossParameters.SpacingY, &DefaultPLParams) == false) {
							SuccessfulRead = false;
						}
					}
					// Read the path loss z spacing
					else if (strcmp(ParameterName, "pl_spacing_z") == 0) {
						if (ReadFloat(&Context, &PathLossParameters.SpacingZ, &DefaultPLParams) == false) {
							SuccessfulRead = false;
						}
					}
...
void DisplayConfigParameters(void)
{
	...
	sprintf_s(Buffer, BufferSize, "%.2e", AbsoluteThreshold);
	DisplayParameter("Absolute threshold", Buffer, DefaultAbsoluteThreshold);
