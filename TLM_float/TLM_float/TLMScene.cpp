...
Polygon *FindIntersection(Polygon *A, Polygon *B, float Thickness);
void AddVerticalPolygon(Polygon *VPolygon, float Thickness, float Permittivity, bool PropagateFlag);
void AddHorizontalPolygon(Polygon *HPolygon, float Thickness, float Permittivity, bool PropagateFlag);
void FillTriangle(xyCoordinate P1, xyCoordinate P2, xyCoordinate P3, float Z, float Thickness, float Permittivity);
...
void AddHorizontalPolygon(Polygon *HPolygon, float Thickness, float Permittivity, bool PropagateFlag)
{
	...
	Angles = (float*)malloc(nVertices*sizeof(float));
...
void FillTriangle(xyCoordinate P1, xyCoordinate P2, xyCoordinate P3, float Z, float Thickness, float Permittivity)
{
	...
	if (Sides[0].yCoeff != 0) {
		...
		for (int x = xMin; x <= xMax; x++) {
			float y1 = YFromX(Sides[0],x*GridSpacing);
			float y2 = YFromX(Sides[1],x*GridSpacing);
	...
	if (Sides[2].yCoeff != 0) {
		...
		for (int x = xMin; x <= xMax; x++) {
			float y1 = YFromX(Sides[1],x*GridSpacing);
			float y2 = YFromX(Sides[2],x*GridSpacing);
			yMin = RoundUpwards(MIN(y1,y2)/GridSpacing);
			yMax = (int)(MAX(y1,y2)/GridSpacing);
