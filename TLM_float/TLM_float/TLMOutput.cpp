...
{	
	...
	if (fopen_s(&PathLossFile, FilenameBuffer, "w") != 0) {
		...
		switch (PathLossParameters.Type) {
			...
			case ROUTE: {
				float length = sqrt(SQUARE(PathLossParameters.X2 - PathLossParameters.X1)+SQUARE(PathLossParameters.Y2 - PathLossParameters.Y1));
				float nSamples = length/PathLossParameters.Spacing;
				float dx = (PathLossParameters.X2 - PathLossParameters.X1)/nSamples;
				float dy = (PathLossParameters.Y2 - PathLossParameters.Y1)/nSamples;
			...
			case GRID: {
				float nSamplesX = (PathLossParameters.X2-PathLossParameters.X1)/PathLossParameters.Spacing;
				float nSamplesY = (PathLossParameters.Y2-PathLossParameters.Y1)/PathLossParameters.SpacingY;
				float dx = (PathLossParameters.X2 - PathLossParameters.X1)/nSamplesX;
				float dy = (PathLossParameters.Y2 - PathLossParameters.Y1)/nSamplesY;
			...
			case CUBE: {
				float nSamplesX = (PathLossParameters.X2-PathLossParameters.X1)/PathLossParameters.Spacing;
				float nSamplesY = (PathLossParameters.Y2-PathLossParameters.Y1)/PathLossParameters.SpacingY;
				float nSamplesZ = (PathLossParameters.Z2-PathLossParameters.Z1)/PathLossParameters.SpacingZ;
				float dx = (PathLossParameters.X2 - PathLossParameters.X1)/nSamplesX;
				float dy = (PathLossParameters.Y2 - PathLossParameters.Y1)/nSamplesY;
				float dz = (PathLossParameters.Z2 - PathLossParameters.Z1)/nSamplesZ;
...
{
	CurrentSet->V = (float*)malloc(xSize*sizeof(float));
	for (int x = 0; x < xSize; x++) {
		...
		for (int xx = 0; xx < 10; xx++) { 
			for (int yy = 0; yy < 10; yy++) {
				for (int zz = 0; zz < 10; zz++) {
					...
					d = 9*sqrt((float)(SQUARE(xx)+SQUARE(yy)+SQUARE(zz)));
					fprintf(OutputFile,"%f\t%f\t%f\t%f\n", Grid[x][y][z].Vmax, d, asin((9.0*zz)/d), atan((float)(yy)/xx));
				}
			}
		}
