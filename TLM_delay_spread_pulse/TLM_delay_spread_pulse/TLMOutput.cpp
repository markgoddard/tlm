	...
	if (fopen_s(&PathLossFile, FilenameBuffer, "w") != 0) {
		...
		switch (PathLossParameters.Type) {
			...
			case POINT: {
				...
				AvgDelay = Grid[x][y][z].Em_max/Grid[x][y][z].Emax;
				DelaySpread = GridSpacing/SPEED_OF_LIGHT * sqrt(Grid[x][y][z].Emm_max/Grid[x][y][z].Emax - SQUARE(AvgDelay));
				AvgDelay *= GridSpacing/SPEED_OF_LIGHT;
				PathLoss = VoltageToDB(SPEED_OF_LIGHT/Frequency * sqrt(Grid[x][y][z].Emax) * KAPPA/4/M_PI/GridSpacing);
				fprintf(PathLossFile, "%f\t%f\t%f\t%f\t%f\n", x*GridSpacing, y*GridSpacing, PathLoss, 1E9*AvgDelay, 1E9*DelaySpread);
				break;
			}
