...
extern bool DefaultThreads;
extern bool DefaultSets;
		...
		do {
			if (fgets(Buffer, 200, InputFile) != NULL) {
				...
				if (ParameterName != NULL) {
					...
					else if (strcmp(ParameterName, "threads") == 0) {
						if (ReadInt(&Context, &Threads, &DefaultThreads) == false) {
							SuccessfulRead = false;
						}
					}
					// Read the number of active sets
					else if (strcmp(ParameterName, "active_sets") == 0) {
						if (ReadInt(&Context, &Sets, &DefaultSets) == false) {
							SuccessfulRead = false;
						}
					}
					...
					// Read the output path loss flag
						...
						if (ReadString(&Context, &PathLossString, &DefaultPLParams) == false) {
							...
							if (strcmp(PathLossString, "none") == 0) {
								...
								PathLossParameters.Type = PL_POINT;
							}
...
void DisplayConfigParameters(void)
{
	...
	sprintf_s(Buffer, BufferSize, "%d", Threads);
	DisplayParameter("Number of Threads", Buffer, DefaultThreads);

	// Display the number of active sets
	sprintf_s(Buffer, BufferSize, "%d", Sets);
	DisplayParameter("Number of Active Sets", Buffer, DefaultSets);
	
	// Display the path loss threshold
	sprintf_s(Buffer, BufferSize, "%.2e", MaxPathLoss);
	...
	if (PathLossParameters.Type != NONE) {
		...
		if (PathLossParameters.Type != PL_POINT) {
