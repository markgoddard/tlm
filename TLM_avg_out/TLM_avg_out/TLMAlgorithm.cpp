	...
	for (int x=0; x<xSize; x++) {
		for (int y=0; y<ySize; y++) {
			for (int z=0; z<zSize; z++) {
				Grid[x][y][z].Vmax = sqrt(Grid[x][y][z].Vavg);
			}
		}
	}
#endif // POWER
...
#ifdef RMS
		Average = sqrt(SQUARE(Value) + SQUARE(Grid[x][y][z].V));
#endif // RMS

#ifdef IIR
		Average = (abs(Value) + Grid[x][y][z].Vavg)/2.0;
		Grid[x][y][z].Vavg = Average;
#endif // IIR

#ifdef RMS_IIR
		Average = sqrt(SQUARE(Value) + SQUARE(Grid[x][y][z].Vavg))/2.0;
		Grid[x][y][z].Vavg = Average;
#endif // RMS_IIR

#ifdef POWER
		Average = abs(Value) + abs(Grid[x][y][z].V);
		Grid[x][y][z].Vavg += SQUARE(Value);
#endif // POWER
		
		// Check if this is the highest value that has been seen at this node
		if (Average > Grid[x][y][z].Vmax) {
...
void EvaluateSource(int Iteration)
{
	...
	if (Average > Grid[ImpulseSource.X][ImpulseSource.Y][ImpulseSource.Z].Vmax) {
		...
		Grid[ImpulseSource.X][ImpulseSource.Y][ImpulseSource.Z].Vavg = Average/2;
	}
}
