...
static message_t MessageA = NO_MSG;
static message_t MessageB = NO_MSG;
...
void MainLoop(void)
{
	...
	while (ActiveSetA != NULL || ActiveSetB != NULL) {
		...
		do {
			...
			MessageBuffer = MessageA;
			MessageA = NO_MSG;
			ReleaseMutex(hMutexA);
		}
		while (MessageBuffer != SCATTER);
