...
void SingleIteration(ActiveNode **pActiveSet) 
{
	...
	while (CurrentNode != NULL) {
		...
		if (Grid[x][y][z].RT == NULL) {
			...
			// Positive x-direction
			NodeReference->VxpIn = NodeReference->RT->Rxp*NodeReference->VxpOut;
			if (x < (xSize-1)) {
				NodeReference->VxpIn += Grid[x+1][y][z].VxnOut * NodeReference->RT->Txp;
			}

			// Negative x-direction
			NodeReference->VxnIn = NodeReference->RT->Rxn*NodeReference->VxnOut;
			if (x > 0) {
				NodeReference->VxnIn += Grid[x-1][y][z].VxpOut * NodeReference->RT->Txn;
			}

			// Positive y-direction
			NodeReference->VypIn = NodeReference->RT->Ryp*NodeReference->VypOut;
			if (y < (ySize-1)) {
				NodeReference->VypIn += Grid[x][y+1][z].VynOut * NodeReference->RT->Typ;
			}

			// Negative y-direction
			NodeReference->VynIn = NodeReference->RT->Ryn*NodeReference->VynOut;
			if (y > 0) {
				NodeReference->VynIn += Grid[x][y-1][z].VypOut * NodeReference->RT->Tyn;
			}

			// Positive z-direction
			NodeReference->VzpIn = NodeReference->RT->Rzp*NodeReference->VzpOut;
			if (z < (zSize-1)) {
				NodeReference->VzpIn += Grid[x][y][z+1].VznOut * NodeReference->RT->Tzp;
			}

			// Negative z-direction
			NodeReference->VznIn = NodeReference->RT->Rzn*NodeReference->VznOut;
			if (z > 0) {
				NodeReference->VznIn += Grid[x][y][z-1].VzpOut * NodeReference->RT->Tzn;
			}
		}
		...
		if (abs(Value) > abs(Grid[x][y][z].Vmax)) {
			Grid[x][y][z].Vmax = Value;
		}

		// Assign to the node
		Grid[x][y][z].V = Value;

		if (abs(Value) < AbsoluteThreshold || abs(Value) < abs(NodeReference->Vmax)*RelativeThreshold) {
...
void EvaluateSource(int Iteration)
{
	...
	if (abs(V) > abs(Grid[ImpulseSource.X][ImpulseSource.Y][ImpulseSource.Z].Vmax)) {
		Grid[ImpulseSource.X][ImpulseSource.Y][ImpulseSource.Z].Vmax = V;
	}
}
