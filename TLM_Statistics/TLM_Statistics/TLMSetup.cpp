...
extern bool DefaultStatsFilename;
		...
		do {
			if (fgets(Buffer, 200, InputFile) != NULL) {
				...
				if (ParameterName != NULL) {
					...
					// Read the statistics file name
					else if (strcmp(ParameterName, "stats_filename") == 0) {
						if (ReadString(&Context, &StatsFilename, &DefaultStatsFilename) == false) {
							SuccessfulRead = false;
						}
					}
					...
					// Read the stats flag
					else if (strcmp(ParameterName, "print_statistics") == 0) {
						if (ReadBool(&Context, &InputData.PrintStats.Flag, &InputData.PrintStats.Default) == false) {
							SuccessfulRead = false;
						}
					}
...
void DisplayConfigParameters(void)
{
	...
	DisplayParameter("Max path loss", Buffer, DefaultMaxPathLoss);
	...
	if (InputData.PrintStats.Flag == true) {
		// Display the statistics filename
		DisplayParameter("Statistics filename", StatsFilename, DefaultStatsFilename);
	}
